  ...
  useEffect(() => {
    ...
    const fetchStatistics = async () => {
      try {
        ...
        data.mostReservedCapacities.sort((a, b) => b.reservations - a.reservations);
    ...
    <div className="bg-light dark:bg-gray-900 dark:text-white w-full min-h-screen pl-10 flex flex-col">
      ...
      {isDataAvailable ? (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 flex-grow">
          ...
          <div className="flex flex-col gap-4">
            ...
            <div className="p-8 bg-white dark:bg-gray-800 rounded shadow-md flex flex-col items-center justify-center">
              <h2 className="text-lg font-bold">Total des réservations confirmées</h2>
              <p className="text-lg font-semibold text-gray-600 dark:text-gray-300">Mois de {currentMonth}</p>
              <p className={`text-7xl font-semibold my-5 ${statistics?.statusCounts.confirmed > 0 ? 'text-green-600' : 'text-black dark:text-gray-400'}`}>
                {statistics?.statusCounts.confirmed || 0}
              </p>
            </div>
            ...
            <div className="p-6 bg-white dark:bg-gray-800 rounded shadow-md flex flex-col items-center">
              <h2 className="text-lg font-bold mb-4">Réservations par statut</h2>
              <ul className="mt-4 space-y-1 text-lg flex items-center justify-center gap-8 mt-4 mb-7 font-bold">
                <li className="text-gray-700 dark:text-gray-300 flex items-center gap-2">
                <FaCheckCircle color="#4CAF50"/> Confirmées : <span className="font-semibold text-green-700 dark:text-green-500">{statistics?.statusCounts.confirmed || 0}</span>
                </li>
                <li className="text-gray-700 dark:text-gray-300 flex items-center gap-2">
                  <FaClock color="#FF9800" />En attente : <span className="font-semibold text-orange-500">{statistics?.statusCounts.waiting || 0}</span>
                </li>
                <li className="text-gray-700 dark:text-gray-300 flex items-center gap-2">
                  <FaTimesCircle  color="#F44336"/>No show : <span className="font-semibold text-red-500">{statistics?.statusCounts.canceled || 0}</span>
                </li>
              </ul>
              ...
              <table className="w-full text-left table-auto dark:text-gray-900 dark:bg-gray-800">
                <thead className="dark:bg-gray-800">
                  <tr className="bg-gray-200 dark:bg-gray-900">
                    <th className="px-4 py-2 dark:bg-gray-900 dark:text-white text-center">Tables</th>
                    <th className="px-4 py-2 dark:bg-gray-900 dark:text-white text-center">Réservations</th>
                  </tr>
                </thead>
                <tbody>
                  {statistics?.mostReservedTables.map((table, index) => (
                    <tr key={index} className={index % 2 === 0 ? "bg-gray-100 dark:bg-dark-800" : "dark:bg-dark-700"}>
                      <td className="border dark:border-gray-700 dark:text-white text-center px-4 py-2 text-center">{table.tableNumber || 'Non spécifié'}</td>
                      <td className="border dark:border-gray-700 dark:text-white px-4 py-2 text-center">{table.reservations}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
              ...
              <table className="w-full text-left table-auto dark:text-gray-200">
                <thead>
                  <tr className="bg-gray-200 dark:bg-dark-600">
                    <th className="px-4 py-2 dark:bg-gray-900 dark:text-white text-center">Couverts</th>
                    <th className="px-4 py-2 dark:bg-gray-900 dark:text-white text-center">Réservations</th>
                  </tr>
                </thead>
                <tbody>
                  {statistics?.mostReservedCapacities.map((capacity, index) => (
                    <tr key={index} className={index % 2 === 0 ? "bg-gray-100 dark:bg-dark-800" : "dark:bg-dark-700"}>
                      <td className="border dark:border-gray-700 px-4 py-2 text-center">{capacity.persons} couverts</td>
                      <td className="border dark:border-gray-700 px-4 py-2 text-center">{capacity.reservations}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
        ...
        <div className="flex-grow flex flex-col items-center justify-center text-center">
          ...
          <article className="text-xl pt-1 dark:text-gray-400">Les données seront consultables dès qu'une réservation sera faite sur votre restaurant</article>
        </div>
      )}
    </div>
  );
